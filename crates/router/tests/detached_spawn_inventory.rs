...
//! ending mid-correlation with no marker to say why. That cost 16 of 26 blocking
//! divergences in tape `rec-07df515-08260830-jd`.
//!
...
//! This test checks that directly — it inspects the argument of every
//! `tokio::spawn` in the crates that serve a request, and fires only when one is
//! bare. Adding a correctly instrumented spawn needs no change here.
//!
...
//!
//! # What this does not do, and the deferral behind it
//!
//! Carrying the request span attributes the work. It does not give the child a
//! span of its OWN, so its boundaries have no distinct span path and address
//! below rank 2 (`SpanPath`), which is where deja resolves essentially every
//! call in practice. That is not a regression — uncorrelated is strictly worse —
//! but it is a deliberate deferral, not an oversight: giving these sites named
//! `#[instrument]` spans is address-ADDITIVE and can be measured on its own,
//! and it does not belong in the same change as the attribution fix.
//!
//! It also says nothing about work spawned by a dependency, by the framework, or
//! by a task that itself carried no span, and nothing about work outliving the
//! process. Capturing everything a request eventually causes is not decidable.
//! Announcing a correlation's captured extent in the tape needs a sink marker
//! deja does not have yet, and is tracked there.
